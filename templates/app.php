...
<body class="bg-gray-100 h-screen overflow-hidden text-slate-800 font-sans">
    ...
    <div id="app" v-cloak class="flex h-full w-full flex-col md:flex-row">
        ...
        <div class="flex-1 flex flex-col h-full bg-white relative border-r border-gray-200">
            ...
            <div class="px-4 py-3 border-b bg-white overflow-x-auto whitespace-nowrap no-scrollbar shadow-sm z-20 shrink-0">
                <button @click="setCategory('all')" 
                    :class="currentCategory==='all' ? 'bg-blue-600 text-white shadow-md border-blue-600' : 'bg-white text-slate-600 hover:bg-gray-50 border-gray-200'" 
                ...
                <button v-for="c in categories" :key="c.slug" @click="setCategory(c.slug)" 
                    :class="currentCategory===c.slug ? 'bg-blue-600 text-white shadow-md border-blue-600' : 'bg-white text-slate-600 hover:bg-gray-50 border-gray-200'" 
                    class="px-5 py-1.5 rounded-full text-xs font-bold mr-2 transition-all border">
                    {{c.name}}
                </button>
            </div>
        ...
        <div :class="showCart?'translate-y-0':'translate-y-full md:translate-y-0'" class="fixed md:static inset-0 md:inset-auto z-40 w-full md:w-[400px] bg-white border-l shadow-2xl md:shadow-none flex flex-col transition-transform duration-300">
                    ...
                    <div class="flex-1 min-w-0 flex flex-col justify-between">
                        ...
                        <div class="flex justify-between items-end">
                            ...
                            <div class="flex items-center bg-gray-50 rounded-lg border">
                                ...
                                <button @click="qty(i,1)" class="w-8 h-7 font-bold hover:text-blue-500">+</button>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="p-6 border-t bg-slate-50 shrink-0 shadow-lg z-20">
                ...
                <div class="flex justify-between items-center mb-6">
                    ...
                    <span class="font-extrabold text-3xl text-blue-600 tracking-tight">{{fmt(grandTotal)}}</span>
                </div>
                <button @click="modal=true" :disabled="!cart.length" class="w-full py-3.5 bg-slate-900 text-white rounded-xl font-bold hover:bg-slate-800 disabled:opacity-50 transition shadow-xl flex justify-center items-center gap-2">
            ...
            <div class="bg-white w-full md:max-w-md rounded-t-2xl md:rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
                ...
                <div class="p-6 overflow-y-auto bg-gray-50">
                    ...
                    <div class="grid grid-cols-2 gap-4 mb-6">
                        <button @click="method='cash'" :class="method==='cash'?'ring-2 ring-blue-500 bg-white shadow-md':'bg-gray-100 text-gray-500'" class="p-4 rounded-xl font-bold text-center transition border border-transparent">Tunai</button>
                        <button @click="method='qris'" :class="method==='qris'?'ring-2 ring-blue-500 bg-white shadow-md':'bg-gray-100 text-gray-500'" class="p-4 rounded-xl font-bold text-center transition border border-transparent">QRIS</button>
                    </div>
                    <div v-if="method==='cash'" class="mb-6">
                        <div class="relative mb-3"><span class="absolute left-4 top-3.5 font-bold text-lg text-gray-400">Rp</span><input type="number" v-model="paid" ref="cashInput" class="w-full pl-12 p-3 border border-gray-300 rounded-xl text-xl font-bold focus:ring-2 focus:ring-blue-500 outline-none" placeholder="0"></div>
                ...
                <div class="p-5 border-t bg-white">
                    <button @click="checkout" :disabled="processing||(method==='cash'&&change<0)" class="w-full py-4 bg-blue-600 text-white rounded-xl font-bold hover:bg-blue-700 disabled:opacity-50 shadow-lg transition flex justify-center gap-2"><i v-if="processing" class="ri-loader-4-line animate-spin text-xl"></i> {{method==='qris'?'Konfirmasi Selesai':'Bayar & Cetak'}}</button>
                </div>
            </div>
        </div>
        ...
        <div id="receipt-print" class="hidden">
            ...
            <div class="receipt-body">
                ...
                <div style="margin-top:15px;text-align:center">Terima Kasih!</div>
            </div>
        </div>
    </div>
