...
<head>
    ...
    <title>Kresuber POS v1.6.1</title>
    ...
    <div id="app" v-cloak class="flex h-full w-full flex-col md:flex-row">
        ...
        <div class="flex-1 flex flex-col h-full bg-white relative border-r border-gray-200">
            ...
            <div class="hidden md:flex h-16 px-6 border-b justify-between items-center z-30 shrink-0 bg-white">
                <div class="flex items-center gap-6 w-full max-w-3xl">
                    ...
                    <div class="relative w-full max-w-md group">
                        ...
                        <input v-model="search" type="text" placeholder="Cari Produk / Scan (F3)" class="w-full pl-10 pr-12 py-2 bg-gray-100 rounded-lg outline-none focus:ring-2 focus:ring-blue-500 transition border-transparent border focus:bg-white">
                        <!-- Camera Scan Button -->
                        <button @click="openScanner" class="absolute right-2 top-1.5 p-1 text-gray-500 hover:text-blue-600 hover:bg-white rounded-md transition" title="Scan Kamera">
            ...
            <div class="md:hidden h-14 bg-white border-b flex items-center justify-between px-4 z-50 shrink-0">
                ...
                <div class="flex items-center gap-3">
                    <button @click="openScanner" class="p-2 bg-gray-100 rounded-full text-blue-600"><i class="ri-qr-scan-2-line text-xl"></i></button>
            ...
            <!-- FIX: Category Chips (Active State) -->
            <div class="px-4 py-3 border-b bg-white overflow-x-auto whitespace-nowrap no-scrollbar shadow-sm z-20 shrink-0">
                <button @click="setCategory('all')" 
                    :class="curCat==='all' ? 'bg-blue-600 text-white shadow-md border-blue-600' : 'bg-white text-slate-600 hover:bg-gray-50 border-gray-200'" 
                ...
                <button v-for="c in categories" :key="c.slug" @click="setCategory(c.slug)" 
                    :class="curCat===c.slug ? 'bg-blue-600 text-white shadow-md border-blue-600' : 'bg-white text-slate-600 hover:bg-gray-50 border-gray-200'" 
                    class="px-5 py-1.5 rounded-full text-xs font-bold mr-2 transition-all border">
                    {{c.name}}
                </button>
            </div>
            ...
            <div v-if="viewMode==='pos'" class="flex-1 overflow-y-auto p-4 md:p-6 bg-slate-50 custom-scrollbar">
                ...
                <div v-else class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4 pb-20">
                    <div v-for="p in products" :key="p.id" @click="addToCart(p)" class="bg-white rounded-xl shadow-sm hover:shadow-md cursor-pointer overflow-hidden border border-transparent hover:border-blue-500 flex flex-col h-64 transition group">
                        ...
                        <div class="p-3 flex flex-col flex-1 justify-between">
                            ...
                            <div class="flex justify-between items-center mt-1">
                                ...
                                <div class="w-8 h-8 rounded-full bg-gray-100 flex items-center justify-center group-hover:bg-blue-600 group-hover:text-white transition"><i class="ri-add-line"></i></div>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div v-if="viewMode==='orders'" class="flex-1 overflow-y-auto p-6 bg-slate-50 custom-scrollbar">
                <div class="flex justify-between items-center mb-6">
                    ...
                    <button @click="viewMode='pos'" class="px-4 py-2 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 font-bold text-sm flex items-center gap-2 transition">
                        <i class="ri-arrow-left-line"></i> Kembali
                    </button>
                </div>
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                    <table class="w-full text-sm text-left">
                        <thead class="bg-gray-50 text-gray-500 border-b"><tr><th class="p-4">ID</th><th class="p-4">Waktu</th><th class="p-4">Items</th><th class="p-4 text-right">Total</th><th class="p-4 text-center">Status</th></tr></thead>
                        <tbody class="divide-y divide-gray-100">
                            <tr v-for="o in recentOrders" :key="o.id" class="hover:bg-blue-50">
                                ...
                                <td class="p-4"><div v-for="i in o.items" class="text-xs text-gray-700">{{i.qty}}x {{i.name}}</div></td>
        ...
        <div :class="showCart?'translate-y-0':'translate-y-full md:translate-y-0'" class="fixed md:static inset-0 md:inset-auto z-40 w-full md:w-[400px] bg-white border-l shadow-2xl md:shadow-none flex flex-col transition-transform duration-300">
            <div class="px-5 py-4 border-b flex justify-between items-center bg-white shrink-0">
                <h2 class="font-bold text-lg flex items-center gap-2"><i class="ri-shopping-cart-2-fill text-blue-600"></i> Keranjang</h2>
            ...
            <div class="p-6 border-t bg-slate-50 shrink-0 shadow-lg z-20">
                <div class="flex justify-between items-center mb-6"><span class="font-bold text-lg text-slate-800">Total</span><span class="font-extrabold text-3xl text-blue-600 tracking-tight">{{fmt(grandTotal)}}</span></div>
                <button @click="modal=true" :disabled="!cart.length" class="w-full py-3.5 bg-blue-600 text-white rounded-xl font-bold shadow-lg hover:bg-blue-700 transition disabled:opacity-50 flex justify-center items-center gap-2"><i class="ri-secure-payment-line"></i> Bayar Sekarang</button>
            </div>
        </div>
        ...
        <div v-if="showScanner" class="fixed inset-0 z-[70] flex items-center justify-center bg-black/80 backdrop-blur-sm">
             <div class="bg-white w-full max-w-md rounded-2xl p-4 relative">
                 <button @click="closeScanner" class="absolute top-2 right-2 z-10 bg-white rounded-full p-1 text-red-500"><i class="ri-close-circle-fill text-3xl"></i></button>
                 <h3 class="text-center font-bold mb-4">Scan Barcode / QR</h3>
        ...
        <div v-if="modal" class="fixed inset-0 z-[60] flex items-end md:items-center justify-center p-0 md:p-4 bg-slate-900/70 backdrop-blur-sm transition-all">
            <div class="bg-white w-full md:max-w-md rounded-t-2xl md:rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
                ...
                <div class="p-6 overflow-y-auto bg-gray-50">
                    ...
                    <div class="grid grid-cols-2 gap-4 mb-6"><button @click="method='cash'" :class="method==='cash'?'ring-2 ring-blue-500 bg-white shadow-md':'bg-gray-100 text-gray-500'" class="p-4 rounded-xl font-bold text-center transition border border-transparent">Tunai</button><button @click="method='qris'" :class="method==='qris'?'ring-2 ring-blue-500 bg-white shadow-md':'bg-gray-100 text-gray-500'" class="p-4 rounded-xl font-bold text-center transition border border-transparent">QRIS</button></div>
                    <div v-if="method==='cash'" class="mb-6"><div class="relative mb-3"><span class="absolute left-4 top-3.5 font-bold text-lg text-gray-400">Rp</span><input type="number" v-model="paid" ref="cashInput" class="w-full pl-12 p-3 border rounded-xl text-xl font-bold focus:ring-2 focus:ring-blue-500 outline-none" placeholder="0"></div><div class="flex gap-2 overflow-x-auto pb-1 no-scrollbar mb-4"><button v-for="a in quickCash" :key="a" @click="paid=a" class="px-3 py-1.5 bg-white border rounded-lg text-xs font-bold whitespace-nowrap shadow-sm transition">{{fmt(a)}}</button></div><div class="flex justify-between font-bold p-4 bg-white rounded-xl border border-gray-200" :class="change>=0?'text-green-600':'text-red-500'"><span>Kembali</span><span>{{fmt(Math.max(0,change))}}</span></div></div>
                ...
                <div class="p-5 border-t bg-white"><button @click="checkout" :disabled="processing||(method==='cash'&&change<0)" class="w-full py-4 bg-blue-600 text-white rounded-xl font-bold hover:bg-blue-700 disabled:opacity-50 shadow-lg transition flex justify-center gap-2"><i v-if="processing" class="ri-loader-4-line animate-spin text-xl"></i> {{method==='qris'?'Konfirmasi Selesai':'Bayar & Cetak'}}</button></div>
            </div>
        </div>
    </div>
